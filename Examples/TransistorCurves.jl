...
transistor_model(T) = ACMModel(MOSf(T),1.0e-4,1.0e-4)
...
for (i,Tv) in enumerate(LinRange(200,500,5))
    plot!(0:0.05:1.8,x->1e6*Idf(x,0.1,Tv),label="\$T = $(Tv) K\$",linecolor=CList2[i]) # plot Id, Vg characteristics for different Temperatures
end
